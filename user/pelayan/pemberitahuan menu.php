          ...
          <div class="demo-charts mdl-color--white mdl-shadow--2dp mdl-cell mdl-cell--12-col mdl-grid">
            <!-- Form Detail Makanan-->
              ...
              <table class="mdl-data-table mdl-js-data-table mdl-cell mdl-cell--12-col ">
                <tbody>
                <tr>
                  <td class="mdl-data-table__cell--non-numeric">Nama Menu</td>
                  <td class="mdl-data-table__cell--non-numeric">Jumlah</td>
                </tr>
<?php
$query="select * from pesanan where Status='Belum'and id_pegawai='$idpegawai' and date(tgl_pesanan)=date(now())";
$mysql=mysql_query($query);
while($row=mysql_fetch_array($mysql)){
  $id_pesanan=$row['no_pesanan'];
	$query2="select d.*,m.nama_menu from detailpesanan d join menu m on d.id_menu=m.id_menu where d.id_pesanan='$id_pesanan' and d.status='Sudah'";
  $mysql2=mysql_query($query2);
  while($row2=mysql_fetch_array($mysql2)){
?>
                <tr>
                  <td class="mdl-data-table__cell--non-numeric"><?php echo $row2['nama_menu'];?></td>
                  <td class="mdl-data-table__cell--non-numeric"><?php echo $row2['jumlah'];?></td>
                </tr>
<?php }} ?>
              </tbody>
              </table>
