<?php
    ...
    $query = "SELECT date(tgl_pesanan)as tanggal ,count(total_harga) as total FROM pesanan WHERE Status='Bayar' AND month(tgl_pesanan)=month(now()) GROUP by date(tgl_pesanan)";
	...
	#ambil data di tabel dan masukkan ke array
    $query2 = "SELECT count(total_harga) as total FROM pesanan WHERE Status='Bayar' AND month(tgl_pesanan)=month(now())";
